  ...
  <div>
    <el-form ref="ruleForm" :model="form" :rules="rules" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item label="实验名称" prop="exname">
        <el-input v-model="form.exname" clearable />
      </el-form-item>
      ...
      <el-form-item label="专业类别" prop="cid">
        <el-select v-model="form.major">
          <el-option v-for="item in professtion" :key="item.id" :label="item.najorname" :value="item.najorname" @click.native="form.cid = item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="实验简介" prop="exinfo">
        <el-input v-model="form.exinfo" type="textarea" :rows="6" />
      </el-form-item>
